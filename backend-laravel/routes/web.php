...
Route::get('/api/main_categories/{mainCategory}', function (MainCategory $mainCategory) {
    return $mainCategory;
});

Route::post('/api/main_categories', function (Request $request) {
    return MainCategory::create($request->all());
});

Route::put('/api/main_categories/{mainCategory}', function (MainCategory $mainCategory, Request $request) {
    $mainCategory->update($request->all());
    return $mainCategory;
});

Route::delete('/api/main_categories/{mainCategory}', function (MainCategory $mainCategory) {
    $mainCategory->delete();
...
Route::get('/api/sub_categories/{subCategory}', function (SubCategory $subCategory) {
    return $subCategory;
});

Route::post('/api/sub_categories', function (Request $request) {
    return SubCategory::create($request->all());
});

Route::put('/api/sub_categories/{subCategory}', function (SubCategory $subCategory, Request $request) {
    $subCategory->update($request->all());
    return $subCategory;
});

Route::delete('/api/sub_categories/{subCategory}', function (SubCategory $subCategory) {
    $subCategory->delete();
    return response()->json(['message' => 'Deleted']);
});
...
Route::get('/api/transactions/{transaction}', function (Transaction $transaction) {
    return $transaction;
});
...
Route::get('/api/month-summary/{month}', function ($month) {
